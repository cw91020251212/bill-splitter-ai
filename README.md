...
| 部件 | 顏色 | 色碼 |
|---|---|---|
| 背景漸變 | 綠（跟 App 主色） | `#22C55E` → `#117A3A` |
| 收據標題條 | 金黃 | `#F59E0B` |
| 勾號 | 藍 | `#2563EB` |
| 收據紙 / 徽章底 | 白 | `#FFFFFF` |
| 內文線條 | 淺灰綠 | `#C6D5CD` |

16px 細尺寸會自動簡化內部線條（隱藏三條內文線、放大標題條），
確保 favicon 都睇得清。

### 想改色？

改 `icons/icon.svg` 最方便，佢係向量檔，用文字編輯器開就見到色碼。
但要注意 **PNG 檔唔會自動跟住變** — PNG 係另外生成嘅。
如果你想整套重新生成，最簡單係話我知你要咩色，我幫你重出一套齊全嘅。
